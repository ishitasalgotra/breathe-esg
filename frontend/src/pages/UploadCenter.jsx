...
export default function UploadCenter() {
  ...
  const load = () => api.get("/ingestion/uploads/").then(r => setUploads(r.data.results || r.data)).finally(() => setLoading(false));
  useEffect(() => { load(); }, []);
  const submit = async (e) => { e.preventDefault(); if (!file) { showToast("Choose a CSV file before uploading.", "info"); return; } const form = new FormData(); form.append("source_type", sourceType); form.append("file", file); setMessage("Processing upload..."); try { await api.post("/ingestion/uploads/", form); setMessage("Upload processed."); showToast("Upload processed successfully.", "success"); setFile(null); if (inputRef.current) inputRef.current.value = ""; load(); } catch (err) { const error = err.response?.data?.file?.[0] || "Upload failed."; setMessage(error); showToast(error, "error"); } };
