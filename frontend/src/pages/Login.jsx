...
export default function Login() {
  ...
  const submit = async (e) => { e.preventDefault(); setError(""); try { const { data } = await api.post("/auth/login/", { email, password }); saveSession(data); showToast("Signed in successfully.", "success"); navigate("/"); } catch { const message = "Login failed. Check credentials and backend availability."; setError(message); showToast(message, "error"); } };
